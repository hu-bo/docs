...
export class PermissionService {
     ...
     * 获取用户在空间的权限
     */
    async getSpacePermission(username: string, spaceId: string): Promise<SpacePermission> {
        ...
        if (!space) {
            return { canRead: false, canCreateFolder: false, canCreateDoc: false, isSuperAdmin: false }
        }

        // 个人空间只有创建者能访问
        if (space.spaceType === SpaceType.PERSONAL) {
            const isCreator = space.creator === username
            return {
                canRead: isCreator,
        ...
        // 社区空间默认可读
        return { canRead: true, canCreateFolder: false, canCreateDoc: false, isSuperAdmin: false }
    }
     ...
     * 获取用户对文档的权限
     */
    async getDocPermission(username: string, docId: string): Promise<DocPermission> {
        const ds = getDataSource()
        ...
        if (!doc) {
            return { canRead: false, canEdit: false, isOwner: false, isSuperAdmin: false }
        }
        ...
        switch (doc.accessMode) {
            case AccessMode.OPEN_EDIT:
                // 所有人可编辑
                return { canRead: true, canEdit: true, isOwner: false, isSuperAdmin: false }

            case AccessMode.OPEN_READONLY:
                // 默认只读，检查 doc_user_acl 和 doc_space_acl 是否有 EDIT 权限
                const hasUserEditPerm = await this.checkDocUserPerm(username, docId, DocPerm.EDIT)
                const hasSpaceEditPerm = await this.checkDocSpacePerm(username, docId, DocSpacePerm.EDIT)
                return {
                    canRead: true,
            ...
            case AccessMode.WHITELIST_ONLY:
                ...
                const hasSpaceReadPerm = await this.checkDocSpacePerm(username, docId, DocSpacePerm.READ)
                ...
                if (hasSpaceReadPerm) {
                    const hasSpaceEdit = await this.checkDocSpacePerm(username, docId, DocSpacePerm.EDIT)
                    return {
                        canRead: true,
     ...
     */
    private async checkDocUserPerm(username: string, docId: string, minPerm: DocPerm): Promise<boolean> {
        const ds = getDataSource()
     ...
     */
    private async checkDocSpacePerm(username: string, docId: string, minPerm: DocSpacePerm): Promise<boolean> {
        const ds = getDataSource()
    ...
    async canViewFolder(username: string, folderId: string): Promise<boolean> {
        ...
        if (folder.visibilityScope === VisibilityScope.ALL) {
            const spacePerm = await this.getSpacePermission(username, folder.spaceId)
     ...
     * 获取用户可见的目录ID列表
     */
    async getVisibleFolderIds(username: string, spaceId: string): Promise<string[]> {
        const ds = getDataSource()
        ...
        const folders = await ds.getRepository(SpaceFolder).find({
            where: { spaceId, isDeleted: 0 }
        })

        const visibleIds: string[] = ['0'] // 根目录始终可见

        for (const folder of folders) {
            if (folder.visibilityScope === VisibilityScope.ALL) {
                visibleIds.push(folder.documentId)
            } else if (folder.visibilityScope === VisibilityScope.DEPT_ONLY && userSpaceAuth) {
                visibleIds.push(folder.documentId)
            }
        }
     ...
     * 检查是否是文档所有者或空间超管
     */
    async isDocOwnerOrSuperAdmin(username: string, docId: string): Promise<boolean> {
